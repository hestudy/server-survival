...
STATE.world.hooks = {
    ...
    onCacheHit(service, req) {
        STATE.sound.playSuccess();
        service.flashCacheHit();
    },

    // ---- Event system (M1-d) — presentation only. Trigger timing, traffic
    // mix swaps, multipliers and outage target selection all live in the sim
    // (src/sim/events.js); these hooks draw warnings, indicators and sounds.
    onMaliciousWarning() {
    ...
    onTrafficShiftStart(shift) {
        // Shift configs carry only { name, distribution } — presenting
        // shift.name (never a nonexistent shift.type) is the 「流量切换崩溃」
        // regression contract.
    ...
    onEventStart(eventType, detail) {
        switch (eventType) {
            case "COST_SPIKE":
                addInterventionWarning(i18n.t('cost_spike_warning'), "danger", 8000);
                break;
            case "CAPACITY_DROP":
                addInterventionWarning(i18n.t('capacity_drop_warning'), "danger", 8000);
                break;
            case "TRAFFIC_BURST":
                addInterventionWarning(i18n.t('traffic_burst_warning'), "warning", 8000);
                break;
            case "SERVICE_OUTAGE":
                if (detail?.service) {
                    const target = detail.service;
    ...
    onEventEnd(eventType, detail) {
        if (eventType === "SERVICE_OUTAGE") {
    ...
    onRpsMilestone(milestone) {
        addInterventionWarning(
            i18n.t('rps_surge_warning', { multiplier: milestone.multiplier.toFixed(1) }),
            "danger",
            5000
        );
    },
...
// Game-mode policies for the sim: whether upkeep time-scaling applies,
// whether the event system runs (survival, or campaign levels that opt into
// survival shifts) and whether service health degrades (survival only).
STATE.world.economy.upkeepScalingEnabled = () =>
    STATE.gameMode === "survival" && CONFIG.survival.upkeepScaling.enabled;
STATE.world.events.enabled = () => {
    if (STATE.gameMode === "campaign") {
        return !!STATE.campaign?.level?.enableSurvivalShifts;
    }
    return STATE.gameMode === "survival";
};
STATE.world.degradationEnabled = () => STATE.gameMode === "survival";
...
// Target RPS (with milestone acceleration and its one-shot announcements)
// computes in the sim event system; milestones announce via onRpsMilestone.
function calculateTargetRPS() {
    return STATE.world.events.targetRPS();
}
...
// Event timing rides the sim clock (M1-d): pausing stops the clock, so the
// active event — and, for outages, its target service — freezes in place.
// No more end-on-pause / re-trigger-on-resume juggling.
...
function updateActiveEventTimer() {
    ...
    // Sim-clock seconds (M1-d): the countdown freezes while paused and
    // scales with fast-forward, exactly like the event itself.
    const remaining = Math.max(
        0,
        STATE.intervention.eventEndTime - STATE.elapsedGameTime
    );
...
function resetGame(mode = "survival") {
    ...
    // Event system (spike/shift/random events/milestones) back to zero;
    // the web-side warning history resets with it.
    STATE.world.events.reset();
    STATE.intervention.warnings = [];
    STATE.intervention.recentEvents = [];
...
// Health numbers (decay, idle regen, active auto-repair) live in the sim
// core; this wrapper drives the sim pass and refreshes the visuals.
function processAutoRepair(dt) {
    STATE.world.processAutoRepair(dt);
    if (STATE.autoRepairEnabled && STATE.gameMode === "survival") {
        STATE.services.forEach((s) => {
            if (s.health < 100) s.updateHealthVisual();
        });
    }
}
...
// Traffic type drawing, entry selection (CDN → WAF → APIGW → any, with
// round-robin across identical entries) and spawning live in the simulation
// core now — see SimWorld.spawnRequest / routeRequestToEntry.
...
function finishRequest(req, viaServiceType) {
    STATE.world.finishRequest(req, viaServiceType);
}

function failRequest(req) {
    STATE.world.failRequest(req);
}

function throttleRequest(req) {
    STATE.world.throttleRequest(req);
}

function removeRequest(req) {
    STATE.world.removeRequest(req);
}
...
// calculateFailChanceBasedOnLoad moved to the simulation core
// (src/sim/service.js); imported at the top of this file for the tooltip
// code and the window bridge.
...
function animate(time) {
    ...
    // Event system (spike, shift, random events) ticks in the sim core;
...
function loadGameState(saveData = null) {
    try {
        ...
        // Event state starts fresh on load (saves never restored it):
        // timers, active events and multipliers reset in the sim.
        STATE.world.events.reset();
        STATE.intervention.warnings = [];
        STATE.intervention.recentEvents = [];
